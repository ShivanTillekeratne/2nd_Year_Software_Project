...
const Dashboard = () => {
  ...
  const handleCheckValidity = async () => {
    ...
    try {
      ...
      const newValidityResults = response.data;
      const existingResults = JSON.parse(localStorage.getItem('validityResults')) || [];
      const updatedResults = [...existingResults, { timestamp: new Date(), results: newValidityResults }];
      localStorage.setItem('validityResults', JSON.stringify(updatedResults));
            ...
            {claims.length > 0 ? (
              <table className="w-full bg-gray-800 text-white">
                ...
                <tbody>
                  {claims.map((claim) => {
                    return (
                      <tr key={claim.claim_id} className="py-2 px-4 border-b border-gray-600">
