{/* IMPORTS AND SOME CONST GO HERE */}
...
const Dashboard = () => {
  ...
  const handleReverseResult = (claimId) => {
    setReversedResults((prevState) => ({
      ...prevState,
  ...
  const handleFinalizeResults = async () => {
    const finalResults = validityResults.map(result => ({
      ...result,
      // is_fraud: overriddenResults[result.claim_id] !== undefined ? !result.is_fraud : result.is_fraud,
            ...
            <table className="w-full bg-gray-800 text-white">
              <thead>
                <tr>
                  {validityChecked && (
                    <>
                      <th className="py-2 px-4 border-b border-r border-gray-600">Reverse</th>
          ...
          {validityChecked && validityResults.length > 0 && (
            <div className="mt-4">
                ...
                {validityResults.map(result => (
                    <div key={result.claim_id} className="mb-4 p-4 border border-gray-600 rounded-lg text-white">
...
<div className="flex gap-4">
              ...
              <button
                ...
                className={`px-4 py-2 ${validityChecked ? 'bg-blue-600' : 'bg-gray-600'} text-white rounded-md hover:bg-blue-700 transition-colors duration-300`}
