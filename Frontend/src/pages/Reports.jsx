...
const Reports = () => {
  ...
  const fetchMonthlyTrendData = async () => {
    try {
      ...
      const sortedData = response.data.sort((a, b) => {
        const dateA = new Date(a.year, a.month - 1, a.day); // month is 0-indexed in JS Date
  ...
  const handleExportPDF = async () => {
    ...
    const chartElements = [
      { id: 'bar-chart-detanalysis', label: 'Detailed Analysis' },
      { id: 'line-chart-dailytrend', label: 'Daily Claims Trend' },
      { id: 'bar-chart-revclaims', label: 'Reversed Claims Over Time' },
    ];
  
    let currentY = doc.autoTable.previous.finalY + 30;
  
    for (const { id, label } of chartElements) {
