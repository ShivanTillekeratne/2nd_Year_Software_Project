...
scaler_path = os.path.join(BASE_DIR, 'services', 'scaler.pkl')
model_path = os.path.join(BASE_DIR, 'services', 'fraud_detection_model.keras')
...
def make_predictions(valid_claims):
    ...
    # Make predictions
    predictions = model.predict(new_claims_scaled)
